MODEL (
    name staging.stg_sprint_results,
    kind VIEW,
    cron '@daily',
    grain id_spint_result,
    description 'The race results for each driver, at each season round',
    column_descriptions (
        id_sprint_result = 'Unique identifier for the sprint  result, combining season, round, and driver ID',
        ...
        event_type = 'The type of the event, sprint in this table'
    ),
    audits (
...
FROM
    base_table
QUALIFY ROW_NUMBER() OVER(PARTITION BY id_race_result ORDER BY ts_extracted DESC) = 1 -- Possible duplicates in the source table, as it's in append mode